...
from src.utilities.checkpointing import local_path_to_absolute_and_download_if_needed
...
def reload_checkpoint_from_wandb(
    ...
    **reload_kwargs,
    ...
    Args:
        ...
        config_path (str): the path to the config file to be used to reload the model.
                            If None, the config is loaded from Wandb
    ...
    if config_path is not None:
        config_path = local_path_to_absolute_and_download_if_needed(config_path)
        config = OmegaConf.load(config_path)
        config = OmegaConf.unsafe_merge(config, OmegaConf.from_dotlist(override_key_value))
    else:
        config = wandb_api.load_hydra_config_from_wandb(run_path, override_key_value=override_key_value)

    # Find or download the checkpoint
    ...
    # Instantiate model and reload its weights
    try:
