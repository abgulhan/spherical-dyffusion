...
def run_model(config: DictConfig) -> float:
    ...
    wandb_id = config.logger.wandb.get("id") if config.get("logger") and hasattr(config.logger, "wandb") else None
    ...
    if uses_wandb and config.wandb_status == "resume":
        ...
        assert config.get("eval_mode") is None, "eval_mode without wandb is not supported"
        ckpt_path = None
