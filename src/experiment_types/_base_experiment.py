...
class BaseExperiment(LightningModule):
    ...
    def _predict(
        ...
        **kwargs,
        ...
        actual_batch_size = full_batch_size // self.num_predictions  # base_num_predictions
        assert actual_batch_size > 0, f"{actual_batch_size=}, {full_batch_size=}, {self.num_predictions=}"
    ...
    def _get_optim(self, optim_name: str, model_handle=None, **kwargs):
        """
        ...
        # Separate parameters that shouldn't be optimized with weight decay
        decay = []
        no_decay = []
        kwargs_no_decay = kwargs.copy()
        kwargs_no_decay["weight_decay"] = 0
        no_decay_params = {"channel_embed", "pos_embed"}
        ...
        no_grad_params = 0
        for name, m in model_handle.named_parameters():
            if not m.requires_grad:  # Only use parameters that require gradients
                no_grad_params += 1
                continue
            elif any(nd in name for nd in no_decay_params):
                no_decay.append(m)
            else:
                decay.append(m)
        ...
        if no_grad_params == 0:
            self.log_text.info(f"Found {total_params_count} parameters.")
        else:
            self.log_text.info(
                f"Found {total_params_count} parameters, of which {no_grad_params} do not require gradients."
            )

        optim = optimizer(
            [
                {"params": decay, **kwargs},
                {"params": no_decay, **kwargs_no_decay},
            ]
        )
        return optim
    ...
    def on_load_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
        ...
        if self.hparams.reset_optimizer:
            self.log_text.warning("Resetting optimizer states.")
