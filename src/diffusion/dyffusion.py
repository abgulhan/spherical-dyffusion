...
class BaseDYffusion(BaseDiffusion):
    ...
    def __init__(
        ...
        # hack_for_imprecise_interpolation can be used when accidentally using one input-only variable
        # in the list of inputs vs. the list of forcings, which leads to using them twice for the interpolator model,
        # which becomes a problem when using DYffusion, since the forecaster does not predict those input-only variables
        *args,
        **kwargs,
    ):
        ...
        fcond_options = ["data", "none", "data+noise-v1", "data+noise-v2"]
    ...
    def q_sample(
        ...
        **kwargs,
        ...
        if batch_mask is not None:
            ...
            t = t[batch_mask]
            kwargs = {k: v[batch_mask] if isinstance(v, Tensor) else v for k, v in kwargs.items()}
    ...
    def predict_x_last(
        ...
        **kwargs,
        ...
        if dynamic_cond is not None:
            ...
            ), f"{dynamic_cond.shape}[1] != {self.num_timesteps+1}"
    ...
    def sample_loop(
        ...
        **kwargs,
        ...
        progress_bar = tqdm(s_and_snext, desc="Sampling", total=len(sampling_schedule), leave=False)
        ...
        for s, s_next, s_nnext in progress_bar:
            ...
            xhat_th = self.predict_x_last(initial_condition=initial_condition, x_t=x_s, t=s, **kwargs)
            ...
            if self.hparams.sampling_type == "cold":
                if not self.hparams.use_cold_sampling_for_last_step and is_last_step:
                    ...
                    if self.hparams.sampling_type == "cold" or s_nnext > self.num_timesteps - 1:
                        x_s = x_s + d_i1
...
class DYffusion(BaseDYffusion):
    ...
    def __init__(
        ...
        interpolator_local_config_path: Optional[str] = None,  # If set, load the config from this path
        ...
        **kwargs,
        ...
        self.interpolator: InterpolationExperiment = get_checkpoint_from_path_or_wandb(
            ...
            config_path=interpolator_local_config_path,
            wandb_run_id=interpolator_run_id,
            wandb_kwargs=interpolator_wandb_kwargs,
    ...
    def p_losses(self, input_dynamics: Tensor, xt_last: Tensor, **kwargs):
        r"""

        Args:
            input_dynamics: the initial condition data  (time = 0)
            xt_last: the start/target data  (time = horizon)
            **kwargs: may include additional args for models, e.g. static conditions
        """
        _ = kwargs.pop("verbose", None)  # remove verbose from kwargs (unused)
